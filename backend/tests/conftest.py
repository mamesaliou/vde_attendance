...
@pytest.fixture
def sample_attendance(test_db, sample_student, sample_class):
...
@pytest.fixture
def multiple_students(test_db, sample_class):
    """Crée plusieurs étudiants pour les tests"""
    students = []
    for i in range(3):
        student = Student(
            first_name=f"Test{i}",
            last_name=f"Student{i}",
            email=f"test{i}.student@example.com",
            class_id=sample_class.id
        )
        test_db.add(student)
...
@pytest.fixture
def multiple_classes(test_db, sample_teacher):
    """Crée plusieurs classes pour les tests"""
    classes = []
