...
const login = () => {
      ...
      useEffect(() => {
        if (isLoggedIn || localStorage.getItem('access_token')) {
          navigate('/attendance'); // dsi éjà connecté -> on renvoie à l'accueil (ou /dashboard)
        }
      }, [isLoggedIn, navigate]);
       ...
       const handleLogin = async (e) => {
        ...
        try {
          const response = await ance.post(ENDPOINTS.LOGIN, userData)// axios.post('http://127.0.0.1:8000/api/auth/login/', userData)
